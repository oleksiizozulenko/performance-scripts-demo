	...
	<xsl:template match="testResults">
		<html>
			<head>
			   ...
			   {
			      if(document.getElementById(details_id+"_image").className.match("expand"))
			      {
			       document.getElementById(details_id+"_image").className = "collapse";
			         expand(details_id);
			      }
			      else
			      {
			         document.getElementById(details_id+"_image").className = "expand";
			         collapse(details_id);
			      }
               }
			]]></script>
	...
	<xsl:template name="detail">
		...
		<xsl:if test="$allFailureCount > 0">
			...
			<xsl:for-each select="/testResults/*[not(@lb = preceding::*/@lb)]">
				...
				<xsl:variable name="failureCount"
					select="count(../*[@lb = current()/@lb][attribute::s='false'])" />
				<xsl:variable name="current_sub_node" select="position()" />
				<xsl:if test="$failureCount > 0">
					<h3>
					...
					<table class="details" border="0" cellpadding="5"
						...
						<xsl:for-each
							...
							<tr>
								...
								<td valing="top">
									<a href="">
										<xsl:attribute name="href"><xsl:text />javascript:change('sub<xsl:value-of select="$current_sub_node" />_page_details_<xsl:value-of
											select="position()" />')</xsl:attribute>
										<span class="expand">
											<xsl:attribute name="id"><xsl:text />sub<xsl:value-of select="$current_sub_node" />_page_details_<xsl:value-of
												select="position()" />_image</xsl:attribute>
											expand/collapse
										</span>
									</a>
								...
								<td valign="top">
									...
									<div class="page_details">
										<xsl:attribute name="id"><xsl:text />sub<xsl:value-of select="$current_sub_node" />_page_details_<xsl:value-of
											select="position()" /></xsl:attribute>
										<table class="show-data" border="1" bordercolor="#000000">
											<tr>
												<th>ResponseFile</th>
												<td>
											...
											<tr>
												<th>ResponceHeader</th>
												<td>
													<xsl:value-of select="./responceHeader" />
												</td>
											</tr>
											<tr>
												<th>ResponceData</th>
